---
name: db-check
description: Read-only post-operation DB verification for TimeQuest. Walks the user through expected vs actual values across ApplicationUser (Xp/Level), TimeEntry (Status/ApprovedBy/ApprovedAt), BadgeAward, UserTeam, etc., with built-in checks for the approval side-effect chain (Status set, XP delta = Hours×10, Level recalc, new BadgeAwards). SELECT-only; refuses any modification request.
---
