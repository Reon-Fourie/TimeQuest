---
name: hotfix-test
description: Focused hotfix verification on TimeQuest. Fetches the PR diff via `gh pr view` and `gh pr diff` to scope what changed, runs a tight fix-verification + adjacent-area regression check (not a full smoke test), and reports whether the fix is safe to merge / deploy. Read-only on data.
---
